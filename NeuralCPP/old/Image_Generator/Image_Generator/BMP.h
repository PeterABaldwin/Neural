...
struct BMP {
    ...
    void draw_rectangle(uint32_t x0, uint32_t y0, uint32_t w, uint32_t h,
        uint8_t R, uint8_t G, uint8_t B, uint8_t A, uint8_t line_w) {
        if (x0 + w > (uint32_t)bmp_info_header.width || y0 + h > (uint32_t)bmp_info_header.height) {
            throw std::runtime_error("The rectangle does not fit in the image!");
        }

        fill_region(x0, y0, w, line_w, R, G, B, A);                                             // top line
        fill_region(x0, (y0 + h - line_w), w, line_w, R, G, B, A);                              // bottom line
        fill_region((x0 + w - line_w), (y0 + line_w), line_w, (h - (2 * line_w)), R, G, B, A);  // right line
        fill_region(x0, (y0 + line_w), line_w, (h - (2 * line_w)), R, G, B, A);                 // left line
    }
