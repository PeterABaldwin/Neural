...
struct Neuron {
public:
	Neuron(unsigned laya, unsigned numOutputs, unsigned myIndex);//needs to know the number of neurons in the next layer
	...
	vector<double> getWeightFromFile(int lay, int neur);
...
// If change in topology, change values here (later make dynamic)**************************************************************
/**
...
*/
vector<double> Neuron::getWeightFromFile(int lay, int neur) {//c is how many weights per neuron
	...
	case 1:
		numONeur = 16;
		place = 10000;//length of first layer plus one
		if (neur == 0) {
			place += 1;
		}
		else {
	...
	case 2:
		numONeur = 10;
		place = 10017;//length of first layer plus one
		if (neur == 0) {
			place += 1;
		}
		else {
	...
	case 3:
		numONeur = 0;
		place = 10034;//length of first layer plus one
		if (neur == 0) {
			place += 1;
		}
		else {
...
*/
Neuron::Neuron(unsigned laya, unsigned numOutputs, unsigned myIndex) {
	if (fileExists(filename)) {
		vector<double> wei = getWeightFromFile(laya, myIndex);
...
void Net::genNet(const vector<unsigned>& topology) {
	...
	for (unsigned layerNum = 0; layerNum < numLayers; ++layerNum) {//loop to create a new layer in m_layers
		...
		for (unsigned neuronNum = 0; neuronNum <= topology[layerNum]; ++neuronNum) {// <= to include bias
			m_layers.back().push_back(Neuron(layerNum, numOutputs, neuronNum));//.back gives last element
		}
