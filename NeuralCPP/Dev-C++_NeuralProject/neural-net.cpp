...
double Neuron::eta = 0.10;//learner rate (0.0 - slow, 0.2 - medium, 1.0 reckless)
//can experiment with different values
//you can lessen the eta if you have more training data
double Neuron::alpha = 0.4;//momentum (0.0 - no momentum, 0.5 moderate momentum)
//can experiment with different values
...
void Neuron::updateInputWeights(Layer &prevLayer) {
	...
	for (unsigned n = 0; n < prevLayer.size(); ++n) {
		...
		//cout << "old weight: " << neuron.m_outputWeights[m_myIndex].weight << " | old delta: " << oldDeltaWeight << " | //output: " << output << " | " problem was calling output "graident: " << m_gradient << " | "; //to see what values are used
		...
		//cout << "new delta: " << neuron.m_outputWeights[m_myIndex].deltaWeight << " | weight: " << neuron.m_outputWeights[m_myIndex].weight << endl;//to see what weights are cahnged to
	}
}
...
Neuron::Neuron(unsigned numOutputs, unsigned myIndex) {
	for (unsigned c = 0; c < numOutputs; ++c) {//c for connections
		...
		//not in tutorial
		//m_outputWeights.back().deltaWeight = randomWeight();
...
void Net::backProp(const vector<double> &targetVals) {
	...
	for (unsigned layerNum = m_layers.size() - 1; layerNum > 0; --layerNum) {
		...
		//cout << endl << "layer: " << layerNum << " | size: " << layer.size() - 1; // output which layer is being used

		for (unsigned n = 0; n < layer.size() - 1; ++n) {//index each individual neuron
			//cout << endl << "neuron: " << n << endl; // output which neuron in layer is being used
